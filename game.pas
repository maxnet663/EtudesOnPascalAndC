...
function Won(s: star; r: rect; PlrMovesCounter, MovesCounter: word) : boolean;
begin
    Won := (s.x >= r.x) and (s.x < r.x + r.b) and (s.y >= r.y) and 
        (s.y < s.y + r.a) and (PlrMovesCounter < MaxPlayerMoves) and
        (MovesCounter < MaxMoves);
end;
...
begin
    GotoXY(1, 1);
    write('Game moves: ', MovesCounter, '. Your moves: ', PlrMovesCounter);
    if not block then
        write('   NOT blocked')
    else
        write('   Blocked');
